...
const BaseDir 		= path.resolve( __dirname , "./hanlp-1.3.2" );
java.options 		= ['-Xms1024m','-Xmx1024m'];
java.classpath[1] 	= BaseDir + "/src-java";
java.asyncOptions 	= {
...
java.classpath.push( BaseDir + "/hanlp-1.8.4.jar" );
java.classpath.push( BaseDir + "/gson-2.2.2.jar" );
...
module.exports = class HANLP {
	...
	constructor( args ) {
		...
		let opts = this.options = _.extend({
		...
		let Tokenizer = java.import('com.hankcs.hanlp.tokenizer.StandardTokenizer');
	...
	Suggester ( list, words,  Ntop ) {
		...
		list = _.isArray( list ) ? list : new Array( list );
		words = _.isArray( words ) ? words : new Array( words );
